...
| Product imagery | Owner approved publication of the original as an accurate shared Premium Hardwood family image and authorised the sales edit; exact original and derivative hashes are recorded | Approval is complete and `image_approved` is true for both Hardwood records. The asset is not published: copy reviewed bytes into the consuming project, preserve provenance and assign a reviewed public URL before use. It is not thickness-specific proof. |
