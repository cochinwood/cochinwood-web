...
2. `git log --oneline ce24ab15..HEAD` lists **only commits that change this document or the
   gate**. If it lists a commit touching `build.py`, the beacon, or anything else `dist/` is
   made of, then the sha above is stale again and this section has to be redone, not the sha
   alone. AND IF IT LISTS A COMMIT TOUCHING `tools/cutover_preflight.py`, READ THAT DIFF: the
   checklist is what a weakened gate would have to get past, and a gate that checks less than
   it did is exactly as dangerous as a stale sha. Check 5 proves `dist/` still matches the
   pinned commit, so the bytes are safe either way -- what this line protects is the checking.
