...
- Variant selection example: `http://127.0.0.1:8891/commerce-preview/?sku=prem_marine_gurjan_18`
...
**Actual setup** loads `config/catalogue.proposed.json`. The four proposed variants have no approved selling price or stock. Purchasing remains blocked. This view never substitutes the staff rate card or historical prices.
...
- Late quote responses are discarded after the basket, PIN code or mode changes. Order creation locks edits and keeps an idempotency key when retrying an uncertain request.
...
Before any public launch, approved company prices, stock, product specifications and photographs, service areas, freight and policies must replace the incomplete proposed configuration. Bank integration and confirmation, production staff-app integration, authenticated deployment and real Merchant review remain separate launch work. Synthetic fixtures must never be published as retail offers.
