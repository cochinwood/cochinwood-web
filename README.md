...
Sitemaps cover canonical pages, articles and images. A page's `<lastmod>` changes only when its own title, description, structured data or main copy changes: `sitemap_lastmod.py` compares each built page with the same page on the pinned production tree and keeps the published date when nothing changed, otherwise it uses the source revision's date. `tools/seed_sitemap_lastmod.py` corrects, from cf-live history, published dates that predate a later change to a page's title, description or structured data (such as the 15 September Product markup); delete its seed file once `LIVE_SHA` points at a publication built with it. After verifying public release bytes, `tools/submit_indexnow.py` submits changed canonical URLs using the existing public ownership key. It defaults to a dry run and saves receipts to avoid duplicate submissions. Search-engine receipt does not promise indexing or ranking.
